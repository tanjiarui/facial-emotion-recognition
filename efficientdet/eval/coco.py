...
def evaluate(generator, model, threshold=0.01):
    ...
    for index in tqdm(range(generator.size()), desc='COCO evaluation'):
    ...
    # write output
    json.dump(results, open('validation bbox results.json', 'w'), indent=4)
    json.dump(image_ids, open('validation processed image ids.json', 'w'), indent=4)

    # load results in COCO evaluation tool
    coco_true = generator.coco
    coco_pred = coco_true.loadRes('validation bbox results.json')
